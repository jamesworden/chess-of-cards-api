...
public class Game
{
    ...
    public string? GetReasonIfMoveInvalid(Move move, bool asHostPlayer, bool isHostPlayersTurn)
    {
        ...
        if (
            move.PlaceCardAttempts.Select(attempt => attempt.TargetLaneIndex).Distinct().Count() > 1
        )
        {
    ...
    )
    {
        ...
        if (
            (!placedMultipleCards && anyValidOpponentCandidateMoves)
            || !anyValidPlayerCandidateMoves
        )
        {
            SetNextPlayersTurn();
    ...
    )
    {
        return placeCardAttempts
            .SelectMany(placeCardAttempt =>
                PlaceCardAndApplyGameRules(placeCardAttempt, playerIsHost)
            )
            .ToList();
    }
    ...
    )
    {
        ...
        {
            PlaceCard(placeCardAttempt, playerIsHost),
        };
    ...
    )
    {
        ...
        var laneCardsAndRowIndexes = GrabAllCardsFromLane(lane)
            ...
            .ThenBy(cardAndRowIndex =>
                playerIsHost ? cardAndRowIndex.Item2 : -cardAndRowIndex.Item2
            )
            .ToList();
    ...
    private CardMovement PlaceCard(PlaceCardAttempt placeCardAttempt, bool playerIsHost)
    {
        ...
        var player = playerIsHost ? HostPlayer : GuestPlayer;
        var indexInHand = RemoveCardWithMatchingKindAndSuit(
            player.Hand.Cards,
